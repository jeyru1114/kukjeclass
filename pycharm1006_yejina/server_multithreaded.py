...
class Server(threading.Thread):
    ...
    def listen(self):
        ...
        """
        print("리스너 쓰레드가 시작 되었습니다")
        while True:
            try:
                self.lock.acquire() #lock 획득(lock을 획득하고 쓰레드가 일처리하고 lock을 해제해야 다른 쓰레드가 사용할 수 있음
                ...
                self.lock.release() #사용 후 lock 해제
            time.sleep(0.050)

    def receive(self):
        """
            새로운 메시지ㅡㄹㄹ 위한 대기(Listen)
        """
        ...
        if data:
            ...
            if message[0]=='login':
                ...
                if tmp_login !=message[1]:
                    prompt='msq;server;'+message[1]+';Login '+tmp_login\
